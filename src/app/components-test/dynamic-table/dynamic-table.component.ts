...
import { CustomCellDirective } from 'src/app/directives/custom-cell.directive';
import { CustomCellComponent, DynamicComponent, DynamicTableDefinition, DynamicTableGroupingHeader, DynamicTableSearchItem, ItemDetailComponent } from 'src/app/interfaces/index-table';
import { ItemDetailDirective } from 'src/app/directives/item-detail.directive';
...
export class DynamicTableComponent implements OnInit, AfterViewInit, OnDestroy {
    ...
    */
    ...
    /**
     * Listado con los índices de las filas que deben estar seleccionadas por defecto.
     * 
     * Solo aplica en caso de que el flag `useSelection` esté en `true`.
     */
    @Input()
    ...
    ngOnInit(): void {
        ...
        if (this.useSelection) {
            ...
            if (this.defaultSelectedRowIndexes) {
                this._setDefaultSelectedRows();
            }
        }
    ...
    ngAfterViewInit(): void {
        ...
        console.log(this.defaultSelectedRowIndexes);
        ...
        if (this._allRowsSelected) {
            for (let row of this.dataSource.data) {
                if (this.selectionDisabledCallback) {
                    ...
                    this._selectedRows.push(row);
                    // this.defaultSelectedRowIndexes.push(row)
                    // console.log('rellena')
                }
            }
            this.defaultSelectedRowIndexes = []
        }
    ...
    updateRow($event: MatCheckboxChange, row: any): void {
        const length = this._selectedRows.length;
    ...
    private __loadItemDetailComponent__(row: any): void {
        ...
        let itemDetailHost: ItemDetailDirective | undefined  = this._itemDetailDirectiveQueryList?.find(item => item.rowId == getRowIndex())
        /**
         * Busco la directiva asociada a ese row. El _itemDetailDirectiveQueryList no se updatea cuando se usan los 
         * filtros dinamicos, entonces no se puede acceder directamente usando el rowIndex en el QueryList. 
         * Al ItemDetailDirective se le asigna el rowIndex en el html y se asocia contra ese numero.
         * 
         */
        if(this.itemDetailComponent && itemDetailHost && itemDetailHost!.rowId == getRowIndex()){
        ...
        //     if(this.itemDetailComponent && item.rowId == getRowIndex()){
    ...
    private _setDefaultSelectedRows(): void {
        this._selectedRows = this.defaultSelectedRowIndexes.map((index: number) => {
